...
interface DiscordBot {
  ...
  status: string; is_running: boolean; tickets_open: number;
  created_at: string;
}
...
const Admin = () => {
  ...
  const botAction = async (botId: string, botAction: string) => {
    try {
      const { data, error } = await supabase.functions.invoke('admin-auth', {
        body: { action: 'bot_action', bot_id: botId, bot_action: botAction, user_id: user?.id, user_role: user?.role }
      });
      if (error || !data?.success) throw new Error(data?.error || 'Erro');
      toast({ title: `Bot ${botAction === 'start' ? 'ligado' : botAction === 'stop' ? 'desligado' : botAction === 'restart' ? 'reiniciado' : 'excluído'}` });
      fetchData();
    } catch (e: any) { toast({ title: e.message || "Erro", variant: "destructive" }); }
  };
    ...
    <div className="min-h-screen bg-background text-foreground admin-bg bg-noise">
      ...
      <main className="max-w-7xl mx-auto px-4 py-4">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="bg-card/60 border border-border/40 mb-4 h-9 rounded-lg p-0.5 flex-wrap">
          ...
          <TabsContent value="licenses">
            ...
            <div className="flex flex-col md:flex-row gap-2 mb-3">
              <div className="relative flex-1">
                ...
                <Input placeholder="Buscar..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-8 h-8 text-xs bg-card/60 border-border/40 rounded-lg" />
              </div>
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="bg-card/80 border-b border-border/40">
                      ...
                      <th className="text-left px-3 py-2 text-[10px] font-medium text-muted-foreground uppercase hidden lg:table-cell">Expira</th>
                      <th className="text-right px-3 py-2 text-[10px] font-medium text-muted-foreground uppercase">Ações</th>
                    </tr>
                  </thead>
                  <tbody>
                    {isLoading ? (
                      <tr><td colSpan={7} className="text-center py-8"><div className="w-4 h-4 border-2 border-muted-foreground/20 border-t-foreground rounded-full animate-spin mx-auto" /></td></tr>
                    ) : filtered.length === 0 ? (
                      <tr><td colSpan={7} className="text-center py-8 text-muted-foreground text-[11px]">Nenhuma licença</td></tr>
                    ) : filtered.map(l => (
                      <tr key={l.id} className="border-b border-border/20 hover:bg-card/40 transition-colors">
                        ...
                        <td className="px-3 py-2 hidden lg:table-cell">
                          {editingLicense === l.id ? (
                            ...
                            <span className="text-muted-foreground text-[11px]">{fmt(l.expires_at)}</span>
                          )}
                        </td>
                        <td className="px-3 py-2 text-right">
                          {editingLicense === l.id ? (
                            ...
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end" className="bg-card border-border text-xs rounded-lg min-w-[140px]">
                                ...
                                <DropdownMenuSeparator />
                                <DropdownMenuItem onClick={() => delLicense(l.id)} className="text-destructive"><Trash2 className="w-3 h-3 mr-2" /> Excluir</DropdownMenuItem>
                              </DropdownMenuContent>
                            </DropdownMenu>
                          )}
                        </td>
          ...
          <TabsContent value="bots">
            <div className="flex items-center justify-between mb-3">
              <p className="text-[11px] text-muted-foreground">Configure bots Discord individuais para automação de licenças.</p>
            ...
            {bots.length === 0 ? (
              <div className="glass-card p-10 text-center">
                ...
                <p className="text-[10px] text-muted-foreground/60 mt-1">Cadastre um bot para automatizar a gestão de licenças via Discord.</p>
              </div>
            ) : (
          ...
          <TabsContent value="config">
            <div className="glass-card p-4 mb-3">
              ...
              <p className="text-[10px] text-muted-foreground mb-3">"Todos os Eventos" envia para uma única URL.</p>
                  ...
                  <tbody>
                    {staffUsers.map(s => (
                      <tr key={s.id} className="border-b border-border/20 hover:bg-card/40 transition-colors">
                        ...
                        <td className="px-3 py-2 text-muted-foreground hidden lg:table-cell">{fmt(s.last_login)}</td>
                        <td className="px-3 py-2 text-right">
                      ...
                      <table className="w-full text-xs">
                        ...
                        <tbody>
                          {filteredValidation.map(l => (
                            <tr key={l.id} className="border-b border-border/15">
                              <td className="px-3 py-1.5"><span className={`dot ${l.success ? 'dot-active' : 'dot-revoked'}`} /></td>
                              <td className="px-3 py-1.5 font-mono text-muted-foreground">{l.license_key?.slice(0, 16) || '—'}</td>
                              <td className="px-3 py-1.5 font-mono text-muted-foreground">{l.ip_address || '—'}</td>
                              <td className="px-3 py-1.5 text-muted-foreground">{fmt(l.validated_at)}</td>
                            </tr>
                          ))}
                          {filteredValidation.length === 0 && <tr><td colSpan={4} className="text-center py-8 text-muted-foreground text-[11px]">Sem resultados</td></tr>}
                        </tbody>
                      </table>
                    </div>
                  </div>
                    ...
                    <div className="overflow-x-auto max-h-[450px] overflow-y-auto">
                      <table className="w-full text-xs">
                        ...
                        <tbody>
                          {filteredAudit.map(l => (
                            <tr key={l.id} className="border-b border-border/15">
                              <td className="px-3 py-1.5 font-medium">{l.username || '—'}</td>
                              <td className="px-3 py-1.5"><code className="text-[9px] bg-muted px-1 rounded">{l.action}</code></td>
                              <td className="px-3 py-1.5 text-muted-foreground max-w-xs truncate">{l.details || '—'}</td>
                              <td className="px-3 py-1.5 text-muted-foreground">{fmt(l.created_at)}</td>
                            </tr>
                          ))}
                          {filteredAudit.length === 0 && <tr><td colSpan={4} className="text-center py-8 text-muted-foreground text-[11px]">Sem resultados</td></tr>}
                        </tbody>
                      </table>
                    </div>
                  </div>
                  ...
                  <div className="glass-card overflow-hidden">
                    <div className="overflow-x-auto max-h-[450px] overflow-y-auto">
                      <table className="w-full text-xs">
                        ...
                        <tbody>
                          {loginAttempts.map(a => (
                            <tr key={a.id} className="border-b border-border/15">
                              <td className="px-3 py-1.5"><span className={`dot ${a.success ? 'dot-active' : 'dot-revoked'}`} /></td>
                              <td className="px-3 py-1.5 font-medium">{a.username}</td>
                              <td className="px-3 py-1.5 font-mono text-muted-foreground">{a.ip_address || '—'}</td>
                              <td className="px-3 py-1.5 text-muted-foreground">{fmt(a.created_at)}</td>
                            </tr>
                          ))}
                          {loginAttempts.length === 0 && <tr><td colSpan={4} className="text-center py-8 text-muted-foreground text-[11px]">—</td></tr>}
                        </tbody>
                      </table>
                    </div>
                  </div>
